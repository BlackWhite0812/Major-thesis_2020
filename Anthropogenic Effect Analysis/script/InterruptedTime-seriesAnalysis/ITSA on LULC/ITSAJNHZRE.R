...
dataJNHZ <- read.csv('REJNHZV3.csv',header = T)
dataJXHZ <- read.csv('REJXHZV3.csv',header = T)
dataNBHZ <- read.csv('RENBHZV3.csv',header = T)
...
lines(dataJNHZ$time[1:9], fitted(JNmodel_p1)[1:9], col="blue",lwd=2)
...
lines(dataJNHZ$time[10:18], fitted(JNmodel_p1)[10:18], col="blue",lwd=2)
...
segments(10, JNmodel_p1$coef[1] + JNmodel_p1$coef[2]*10 + JNmodel_p1$coef[3]+JNmodel_p1$coef[4]*10 + 
           JNmodel_p1$coef[5] + JNmodel_p1$coef[6],
         18, JNmodel_p1$coef[1] + JNmodel_p1$coef[2]*18 + JNmodel_p1$coef[3]+JNmodel_p1$coef[4]*18 + 
           JNmodel_p1$coef[5] + JNmodel_p1$coef[6]*9,
...
lines(dataJNHZ$time[19:27], fitted(JNmodel_p1)[19:27], col="red",lwd=2)

# Add the second line segment for the control
lines(dataJNHZ$time[28:36], fitted(JNmodel_p1)[28:36], col="red",lwd=2)

# Add the counterfactual for the control group
segments(1, JNmodel_p1$coef[1]+JNmodel_p1$coef[2],
         18,JNmodel_p1$coef[1]+JNmodel_p1$coef[2]*18,
         lty=2,col='red',lwd=2)

# Add in a legend
legend("topleft", legend=c("Jiaxing + Ningbo","Jiaxing","Ningbo","Hangzhou"), col=c("blue","navy","orange","red"),pch=20)
