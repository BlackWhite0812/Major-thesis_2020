...
dataJNHZ <- read.csv('OTJNHZV3.csv',header = T)
dataJXHZ <- read.csv('OTJXHZV3.csv',header = T)
dataNBHZ <- read.csv('OTNBHZV3.csv',header = T)
...
plot(dataJNHZ$time[1:18],dataJNHZ$OT[1:18],
     ylim=c(0,1200),
     ylab="Other area of Impervious Land (km2)",
     xlab="Year",
     pch=20,
     col="white",
     xaxt="n",
     main = "ITSA for other area change in bridge-connected cities 
